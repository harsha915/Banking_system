...
import time
from AHV_Banking.com.service.validations import *
from AHV_Banking.com.service.mail_operations import *
from AHV_Banking.com.service.user_security import *
...
class CreateAccount:
    def accountCreation(self):
        ...
        # inserting user details into database, table ahv_users
        insert_data('ahv_users', first_name=self.fname, last_name=self.lname, email=self.email, dob=self.dob,
                    salt=self.salt, password=self.password)


        # fetching userId of the new user created
        self.userId = fetch_data('ahv_users', email=self.email)

        # inserting account info into database, table ahv_accounts for new user created
        insert_data('ahv_accounts', user_id=self.userId[0][0], account_number=self.accountNumber, balance=self.initialDeposit, is_active=True)
...
def login(user_mail):
    ...
    while True:
        ...
        # checking whether user_mail exist or not in ahv_users
        if user_mail in fetch_column_data('ahv_users', 'email'):
            user_password = input("Password: ")
            print()

            # fetching user data from db using user_mail
            db_data = fetch_data('ahv_users', email=user_mail)
            ...
            if failedLoginAttemps > MAX_LOGIN_ATTEMPTS:
                ...
                if hashed_user_password == db_password:
                    ...
                    # update failedcountattempts to 0 in ahv_users
                    update_data('ahv_users', values={'failedcountattempts': 0}, conditions={'email': user_mail})
                    ...
                    # updating last_login_time in ahv_users to CURRENT TIME
                    ...
                    update_data('ahv_users', values={'failedcountattempts': failedLoginAttemps}, conditions={'email': user_mail})
                    return False
                # else:
...
def forgotPassword():
    ...
    try:
        ...
        # fetching user data row from ahv_users table based on user_mail
        db_data = fetch_data('ahv_users', email=user_mail)
        ...
        if user_dob == str(db_dob):
            ...
            if mailOtpVerification(user_mail):
                ...
                # updating new password, salt into user database in ahv_users table based on user_mail
                update_data('ahv_users', values={'password': updated_password,'salt': updated_salt}, conditions={'email': user_mail})
