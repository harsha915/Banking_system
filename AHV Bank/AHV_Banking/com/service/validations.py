...
#         query = "SELECT COUNT(*) FROM ahv_users WHERE email = %s"
...
def uniqueEmail(emailId):
    ...
    # by fetching email column from ahv_users table
    if emailId in fetch_column_data('ahv_users','email'):
        return False
    else:
        return True
...
#         cursor = conn.cursor()
#         # Execute a query to check if the email already exists
#         query = "SELECT COUNT(*) FROM ahv_accounts WHERE account_number = %s"
...
def uniqueAccNum(AccNum):
    ...
    # by fetching account_number column in ahv_accounts
    if AccNum in fetch_column_data('ahv_accounts','account_number'):
        return False
    else:
        return True
